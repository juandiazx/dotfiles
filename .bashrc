...
# Detect dotfiles directory dynamically (works even if .bashrc is symlinked)
DOTFILES_DIR="$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")" && pwd)"

# Load core dotfiles in root
for file in "$DOTFILES_DIR"/.{aliases,functions}; do
    [ -r "$file" ] && source "$file"
done
...
# Source git-prompt.sh from the system location
